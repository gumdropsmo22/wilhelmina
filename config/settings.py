...
COG_FLAGS: tuple[CogFlag, ...] = (
    CogFlag(extension="cogs.core", env_var="ENABLE_CORE", default_enabled=True, description="Core commands and runtime health checks.", required=True),
    CogFlag(extension="cogs.admin", env_var="ENABLE_ADMIN", default_enabled=True, description="Admin diagnostics and runtime inspection commands.", required=True),
    CogFlag(extension="cogs.help", env_var="ENABLE_HELP", default_enabled=True, description="Living Command Grimoire for public slash-command discovery."),
    CogFlag(extension="cogs.rules", env_var="ENABLE_RULES", default_enabled=True, description="Covenant Gate rules UI and acceptance tracking."),
    CogFlag(extension="cogs.welcome", env_var="ENABLE_WELCOME", default_enabled=False, description="Configured-channel Wilhelmina greeting for new home-guild members."),
    CogFlag(extension="cogs.memory_admin", env_var="ENABLE_MEMORY_ADMIN", default_enabled=True, description="Founder/admin-only persistent Memory Ledger controls."),
    CogFlag(extension="cogs.memory_extraction", env_var="ENABLE_MEMORY_EXTRACTION", default_enabled=False, description="Interaction-scoped automatic Memory Ledger extraction."),
    CogFlag(extension="cogs.chat", env_var="ENABLE_CHAT", default_enabled=False, description="Memory-aware direct-interaction chat routing."),
    CogFlag(extension="cogs.invite", env_var="ENABLE_INVITE", default_enabled=False, description="Invite-link command and Discord authorization helper."),
    CogFlag(extension="cogs.roll", env_var="ENABLE_ROLL", default_enabled=False, description="Dice rolling command with number lore."),
    CogFlag(extension="cogs.eight_ball", env_var="ENABLE_EIGHT_BALL", default_enabled=False, description="Magic 8-Ball question command."),
    CogFlag(extension="cogs.fortune", env_var="ENABLE_FORTUNE", default_enabled=False, description="Fortune-cookie style generated message command."),
    CogFlag(extension="cogs.tarot", env_var="ENABLE_TAROT", default_enabled=False, description="Single-card and Past/Present/Future Tarot readings."),
    CogFlag(extension="cogs.broadcasts", env_var="ENABLE_BROADCASTS", default_enabled=False, description="Scheduled Daily Broadcasts: morning show and evening dispatch."),
)
...
def _get_bool(name: str, *, default: bool = False) -> bool:
    ...
    if value in FALSE_VALUES:
        return False
    raise SettingsError(f"Environment variable {name} must be a boolean (true/false, yes/no, on/off, 1/0), got {raw!r}")
...
def _get_command_sync_mode() -> str:
    ...
    if mode not in VALID_COMMAND_SYNC_MODES:
        raise SettingsError(f"COMMAND_SYNC_MODE must be one of {sorted(VALID_COMMAND_SYNC_MODES)}, got {mode!r}")
    if mode == "dev":
        return "guild"
    return mode
...
def _get_server_mode() -> str:
    ...
    if mode not in VALID_SERVER_MODES:
        raise SettingsError("SERVER_MODE must be 'dedicated'. Server takeover/transformation modes are not supported.")
    return mode


def _read_enabled_cogs() -> Mapping[str, bool]:
    enabled = {flag.extension: _get_bool(flag.env_var, default=flag.default_enabled) for flag in COG_FLAGS}
    if _get_bool("ENABLE_ORACLES", default=False):
        enabled = dict(enabled)
        enabled["cogs.roll"] = True
        enabled["cogs.eight_ball"] = True
        enabled["cogs.fortune"] = True
    disabled_required = [flag.env_var for flag in COG_FLAGS if flag.required and not enabled.get(flag.extension, False)]
    if disabled_required:
        raise SettingsError(f"Required cog flag cannot be disabled: {', '.join(disabled_required)}")
...
def load_settings() -> RuntimeSettings:
    ...
    if command_sync_mode == "guild" and home_guild_id is None:
        raise SettingsError("HOME_GUILD_ID is required when COMMAND_SYNC_MODE=guild (DEV_GUILD_ID is accepted as a legacy alias).")
    return RuntimeSettings(discord_token=discord_token, client_id=client_id, app_env=app_env, server_mode=server_mode, home_guild_id=home_guild_id, database_path=database_path, log_level=log_level, command_sync_mode=command_sync_mode, cog_flags=COG_FLAGS, enabled_cogs=_read_enabled_cogs())
